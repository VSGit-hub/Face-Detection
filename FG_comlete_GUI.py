...
def main():
    ...
    df.to_excel(r'C:\Users\Vaibhav\Desktop\vaibhav\be project\FaceRecog\New_attendance.xlsx', index = True)

    t = Text(root)
    for x in NList:
        t.insert(END, x + '\n')
    t.pack()
btn=tk.Button(root,text="Start Atendance",command=main,padx=5,pady=5,relief=GROOVE,width=15)
btn.pack()
...
def FaceTrain():
    ...
    BASE_DIR =os.path.dirname(os.path.abspath(__file__)) # to fnd the location of faces_train.py in os
    ...
    for root,dirs,files in os.walk(IMAGE_DIR): #  to see images in file images
        for file in files: #itrate through the files ie. images
            if file.endswith("png") or file.endswith("jpg"):
                path=os.path.join(root,file) #path of images
                label=os.path.basename(os.path.dirname(path)).replace(" ","-").lower() #save the base name of director having path
                #print(label,path)
    ...
    messagebox.showinfo("Message","Traning complete")
